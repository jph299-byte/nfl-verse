...
extract_game <- function(path, g) {
  ...
  # For reviewed plays, use the FINAL ruling only (text after the last REVERSED).
  # This matters because a reversal can either REMOVE a fumble or ADD one.
  final_ruling <- function(txt) {
    if (is.na(txt)) return("")
    parts <- strsplit(txt, "(?i)The Replay Official reviewed|[A-Za-z ]+ challenged",
                      perl=TRUE)[[1]]
    # The raw description repeats the final ruling after "REVERSED."
    loc <- gregexpr("(?i)REVERSED\\.", txt, perl=TRUE)[[1]]
    if (loc[1] != -1) {
      last <- tail(loc,1)
      return(substr(txt, last + attr(loc,"match.length")[length(loc)], nchar(txt)))
    }
    txt
  }
  ruling <- vapply(q$description, final_ruling, character(1))

  q$interception <- grepl("INTERCEPTED|intercepted by",ruling,ignore.case=TRUE)
  q$fumble <- grepl("FUMBLES|Fumble",ruling,ignore.case=TRUE)

  # Interception: offensive gain is zero. Defensive return yards do not enter
  # the offensive situation score.
  q$gain[q$interception] <- 0
  q$gain_source <- "raw"
  q$gain_source[q$interception] <- "interception=0"

  # Reconstruct gain from the FINAL ruling. Prefer "to [yardline] for N yards";
  # this prevents recovery/return yardage later in the sentence being used.
  event_idx <- which(q$fumble)
  for (i in event_idx) {
    txt <- ruling[i]
    pre <- sub("(?i)FUMBLES?.*$","",txt,perl=TRUE)

    # Common rush/reception/sack form: "... for N yards ... FUMBLES"
    mm <- gregexpr("for (no gain|-?[0-9]+ yards?)",pre,perl=TRUE,ignore.case=TRUE)
    hits <- regmatches(pre,mm)[[1]]
    if (length(hits) && hits[1] != "-1") {
      h <- tail(hits,1)
      if (grepl("no gain",h,ignore.case=TRUE)) val <- 0
      else val <- as.numeric(sub(".*?(-?[0-9]+).*","\\1",h))
      if (is.finite(val)) {
        q$gain[i] <- val
        q$gain_source[i] <- "pre-fumble final ruling"
      }
    } else if (grepl("FUMBLES \\(Aborted\\)",txt,ignore.case=TRUE)) {
      # For aborted exchanges the NFL raw gain is the net offensive result to
      # the recovery spot. Retain it and expose it explicitly in the audit.
  ...
  q$kneel <- grepl("kneel|kneels",ruling,ignore.case=TRUE)
