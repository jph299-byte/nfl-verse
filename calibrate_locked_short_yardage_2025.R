...
excess8_short_value <- function(down, togo, gain, short_max=0, turnover=FALSE, kneel=FALSE) {
  if (kneel) return(0)

  # Always use the new /8 conversion treatment on successful 3rd/4th downs.
  use_conversion_rule <- down %in% c(3,4) && gain >= togo

  # Candidate extension: successful 2nd-and-short conversions.
  if (down == 2 && short_max > 0 && togo <= short_max && gain >= togo) {
    use_conversion_rule <- TRUE
  }

  if (use_conversion_rule) {
    excess <- max(0, gain-togo)
    v <- min(1.75, 1 + .35*log(1 + excess/8))
    if (turnover) v <- v-1
    return(v)
  }

  # Everything else remains exactly as the current production formula.
  base_value(down,togo,gain,turnover,kneel)
}

score_row <- function(d,t,g,turn,kneel,variant) {
  if (variant == "current") return(base_value(d,t,g,turn,kneel))
  if (variant == "third_fourth_8") return(excess8_short_value(d,t,g,0,turn,kneel))
  short_max <- as.numeric(sub("second_", "", variant))
  excess8_short_value(d,t,g,short_max,turn,kneel)
}

variants <- c("current","third_fourth_8","second_1","second_2","second_3","second_4")
...
for (v in variants) {
  ...
  # Defensive penalty-only 3rd/4th conversion = +1.00.
...
breakdown <- q |>
  mutate(
    bucket = case_when(
      ...
      down == 2 & ydstogo == 3 ~ "2nd & 3",
      down == 2 & ydstogo == 4 ~ "2nd & 4",
      down %in% c(3,4) & ydstogo == 1 ~ "3rd/4th & 1",
      down %in% c(3,4) & ydstogo <= 3 ~ "3rd/4th & 2-3",
      down %in% c(3,4) & ydstogo <= 6 ~ "3rd/4th & 4-6",
      down %in% c(3,4) & ydstogo <= 10 ~ "3rd/4th & 7-10",
      down %in% c(3,4) ~ "3rd/4th & 11+",
      TRUE ~ "other"
    )
  ) |>
  group_by(bucket) |>
  summarise(
    ...
    third_fourth_8=mean(value_third_fourth_8,na.rm=TRUE),
    second_1=mean(value_second_1,na.rm=TRUE),
    second_2=mean(value_second_2,na.rm=TRUE),
    second_3=mean(value_second_3,na.rm=TRUE),
    second_4=mean(value_second_4,na.rm=TRUE),
    .groups="drop"
  )
...
notes <- c(
  ...
  "",
  "Variants: current; third_fourth_8; second_1; second_2; second_3; second_4.",
  "All non-current variants use /8 on successful 3rd/4th conversions.",
  "second_N additionally uses /8 on successful 2nd downs with N or fewer yards to go.",
...
# Current vs /8 on 3rd/4th, then extensions through 2nd & 1-4.
#
...
replay_variants <- c("current", "third_fourth_8", "second_1", "second_2", "second_3", "second_4")
replay_input <- pred |>
