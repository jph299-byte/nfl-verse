...
extract_game <- function(path, g) {
  ...
  # Turnovers from raw NFL descriptions. A fumble counts only when possession
  # is actually lost; interceptions always do.
  q$interception <- grepl("INTERCEPTED|intercepted by",q$description)
  q$fumble_lost <- grepl("FUMBLES.*RECOVERED by|Fumble.*RECOVERED by",
                         q$description,ignore.case=TRUE)
  # Avoid treating an offense recovering its own fumble as a turnover by using
  # explicit "FUMBLES" conservatively; validation output makes these auditable.
  # Raw descriptions often include opponent abbreviation after RECOVERED by.
  # Interceptions are unambiguous.
  q$turnover <- q$interception | q$fumble_lost
  ...
  q$play_value <- mapply(play_value,q$down,q$yards_to_go,q$gain,q$turnover,q$kneel)
